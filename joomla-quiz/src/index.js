...
const correct_speechcon = require('./correct_speechcons');
const correct = require('./correct');
const wrong_speechcon = require('./wrong_speechcons');
...
const APP_ID = 'amzn1.ask.skill.6dd6dfa1-649d-4bba-b233-7106d3152916';

const languageString = {
    'en': {
        'translation': {
            'QUESTIONS': questions['QUESTIONS_EN_US'],
	        'GAME_NAME': '<phoneme alphabet="ipa" ph="d͡ʒʊmla">joomla</phoneme> Quiz', // Be sure to change this for your skill.
            ...
            'NEW_GAME_MESSAGE': 'Welcome to the %s. ',
            'WELCOME_MESSAGE': 'I will ask you %s questions, try to get as many right as you can. ' +
            'Just say the number of the answer. Let\'s begin. ',
            'ANSWER_CORRECT_MESSAGE_SPEECHCONS': correct_speechcon['CORRECT_EN_US'],
            'ANSWER_CORRECT_MESSAGE': correct['CORRECT_EN_US'],
	        'ANSWER_WRONG_MESSAGE_SPEECHCONS': wrong_speechcon['WRONG_EN_US'],
	        'ANSWER_WRONG_MESSAGE': wrong['WRONG_EN_US'],
            'CORRECT_ANSWER_MESSAGE': 'The correct answer is %s: %s ',
            'ANSWER_IS_MESSAGE': '',
            'TELL_QUESTION_MESSAGE': 'Question %s <break time="1s"/> %s <break time="1s"/> ',
            'GAME_OVER_MESSAGE': 'You got %s out of %s questions correct. Thank you for playing!',
            'SCORE_IS_MESSAGE': 'Your score is %s <break time="2s"/>',
        },
    },
    'en-US': {
        'translation': {
    ...
    'en-GB': {
        'translation': {
	        ...
	        'GAME_NAME': '<phoneme alphabet="ipa" ph="d͡ʒʊmla">joomla</phoneme> Quiz',
	        'ANSWER_CORRECT_MESSAGE_SPEECHCONS': correct_speechcon['CORRECT_EN_GB'],
	        'ANSWER_CORRECT_MESSAGE': correct['CORRECT_EN_GB'],
	        'ANSWER_WRONG_MESSAGE_SPEECHCONS': wrong_speechcon['WRONG_EN_GB'],
	        'ANSWER_WRONG_MESSAGE': wrong['WRONG_EN_US'],
    ...
    'de': {
        'translation': {
	        ...
	        'ANSWER_CORRECT_MESSAGE_SPEECHCONS': correct_speechcon['CORRECT_DE_DE'],
	        'ANSWER_CORRECT_MESSAGE': correct['CORRECT_DE_DE'],
	        'ANSWER_WRONG_MESSAGE_SPEECHCONS': wrong_speechcon['WRONG_DE_DE'],
	        'ANSWER_WRONG_MESSAGE': wrong['WRONG_DE_DE'],
	        'CORRECT_ANSWER_MESSAGE': 'Die richtige Antwort ist %s: %s ',
            'ANSWER_IS_MESSAGE': '',
            'TELL_QUESTION_MESSAGE': 'Frage %s <break time="1s"/> %s <break time="1s"/> ',
            'GAME_OVER_MESSAGE': 'Du hast %s von %s richtig beantwortet. Danke fürs Mitspielen!',
            'SCORE_IS_MESSAGE': 'Dein Ergebnis ist %s <break time="2s"/>',
        },
    },
};
...
function populateRoundAnswers(gameQuestionIndexes, correctAnswerIndex, correctAnswerTargetLocation, translatedQuestions) {
    ...
    for (let i = 0; i < ANSWER_COUNT; i++) {
        answers[i] = answersCopy[i] + ' <break time="1s"/> ';
    }
...
function handleUserGuess(userGaveUp) {
	...
	const wrongSpeechcon = this.t("ANSWER_WRONG_MESSAGE_SPEECHCONS", {keySeparator: '#'});
	const correctSpeechcon = this.t("ANSWER_CORRECT_MESSAGE_SPEECHCONS", {keySeparator: '#'});

	const wrongReply = this.t("ANSWER_WRONG_MESSAGE", {keySeparator: '#'});
	const correctReply = this.t("ANSWER_CORRECT_MESSAGE", {keySeparator: '#'});

	const wrongSpeech = '<say-as interpret-as="interjection">' + wrongSpeechcon[Math.floor(Math.random() * wrongSpeechcon.length)] + ' </say-as> <break time="1s"/> ' + wrongReply[Math.floor(Math.random() * wrongReply.length)] + ' <break time="1s"/> ';
	const correctSpeech = '<say-as interpret-as="interjection">' + correctSpeechcon[Math.floor(Math.random() * correctSpeechcon.length)] + ' </say-as> <break time="1s"/> ' +correctReply[Math.floor(Math.random() * correctReply.length)] + ' <break time="1s"/> ' ;


	if (answerSlotValid && parseInt(this.event.request.intent.slots.Answer.value, 10) === this.attributes['correctAnswerIndex']) {
        currentScore++;
        speechOutputAnalysis = correctSpeech;

    } else {
        if (!userGaveUp) {
            speechOutputAnalysis = wrongSpeech;
        }
